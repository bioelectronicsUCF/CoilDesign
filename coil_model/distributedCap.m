...
function [C, C_sub, C_mm_h, C_mm_v] = distributedCap(s, n, od, id, epsilon_r_sub, epsilon_r_mm, epsilon_r_mmf, d, h, t, c_sub_per_area, g_sub_per_area, f)
  ...
  C_mm_v = epsilon_mm / d * w^2 * (1-r(1))^2;
  for i = 2 : n
    C_mm_v = C_mm_v + epsilon_mm / d * w^2 * (1-sum_r(i-1)-r(i)/2)^2;
  end
  ...
  for i = 1 : n-1
    for j = i+1  : i+1
      ...
      C_mm_h_DC_f = pi * epsilon_mmf * Length_in(i) /rr * ellipke((rr^2-1)/rr^2) / ( 1/(rr+1)*(2*(gamma+log(4))*(ellipke((rr-1)^2/(rr+1)^2))-pi*(p_deriv(3,[0.5,0.5,1,(rr-1)^2/(rr+1)^2])+p_deriv(2,[0.5,0.5,1,(rr-1)^2/(rr+1)^2]))) + log((rr+1)/(rr-1))/rr*ellipke((rr^2-1)/rr^2) );
      C_mm_h_DC_a = epsilon_mm / s * t*Length_in(i);
