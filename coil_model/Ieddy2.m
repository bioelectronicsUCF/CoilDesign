function [I_eddy_edge, I_eddy_center] = Ieddy2(od,s,w,delta,H,Sheet_R_coil,omega,n,t)
   ...
   if (w > 2*delta)
     if (t > 2*delta)
       for k=1:n
         ...
         R_e = 2*Sheet_R_coil*2*dc/delta;
         R_c = 2*(Sheet_R_coil*t/(2*delta))*2*dc/(w/2-delta);
         L_e = wheelerTurns(s, n, Dout, Din, t);mu/pi*log((w-delta)/delta);                   sL_e=1i*omega*L_e;
         L_c = mu/pi*log((0.75*w-delta)/(0.25*w-delta));     sL_c=1i*omega*L_eddy_center;
         I_e = (V_e-sM_/(R_c+sL_c)*V_c) / (R_e+sL_e-sM_^2/(R_c+sL_c)) /1e6;
         I_c = (V_c-sM_/(R_e+sL_e)*V_e) / (R_c+sL_c-sM_^2/(R_e+sL_e))/1e6;
