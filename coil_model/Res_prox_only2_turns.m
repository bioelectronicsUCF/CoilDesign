...
function [R_dc, Rprox] = Res_prox_only2_turns(s, n, Dout, Din, Sheet_R_coil, Sheet_R_con, l_con, t, f)
  ...
  for i = 1:n
    for j = 1:n
      if (abs(i-j)==0)
        delH_inc(i) = delH_inc(i) + I_eddy_edge(j)/2.5/pi * (1/(t+delta*2)-1/(w-delta/2));
        delH_inc(i) = delH_inc(i) + I_eddy_center(j)/2.5/pi * (1/(t+w/2)-1/(w-w/4));
  ...
  for i = 1:n
    for j = 1:n
      if (abs(i-j)==0)
        Hin_(i) = Hin_(i) + I_eddy_edge(j)/4/pi * (1/(t+delta*2)-1/(w-delta/2));
        Hin_(i) = Hin_(i) + I_eddy_center(j)/4/pi * (1/(t+w/2)-1/(w-w/4));
        Hout_(i) = Hout_(i) - I_eddy_edge(j)/4/pi * (1/(t+delta*2)-1/(w-delta/2));
        Hout_(i) = Hout_(i) - I_eddy_center(j)/4/pi * (1/(t+w/2)-1/(w-w/4));
      elseif (j==i-1)
        Hout_(i) = Hout_(i) + I_eddy_edge(j)/2/pi * (1/(s+delta/2)-1/(s+w-delta/2));
        Hout_(i) = Hout_(i) + I_eddy_center(j)/2/pi * (1/(w/8+s)-1/(s+w*7/8));
